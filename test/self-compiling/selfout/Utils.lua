...
System.namespace("CSharpLua", function (namespace)
  ...
  namespace.class("Utility", function (namespace)
    ...
    IsFromAssembly = function (symbol)
      return not IsFromCode(symbol)
    end
            ...
            do
              ...
              if isField then
                if IsInterfaceImplementation(symbol, MicrosoftCodeAnalysis.IPropertySymbol) then
                  ...
                  local documentTrivia = Linq.FirstOrDefault(property:GetLeadingTrivia(), function (i)
                    return MicrosoftCodeAnalysis.CSharpExtensions.IsKind(i, 8544 --[[SyntaxKind.SingleLineDocumentationCommentTrivia]])
                  end)
                  if MicrosoftCodeAnalysis.SyntaxTrivia.op_Inequality(documentTrivia, nil) and HasIgnoreAttribute(documentTrivia) then
                    isField = false
                  end
                end
              end
    ...
    HasIgnoreAttribute = function (trivia)
      return trivia:ToString():Contains("@CSharpLua.NoField" --[[LuaDocumentStatement.kNoField]])
    end
    ...
    FillNamespaceName = function (sb, typeSymbol, funcOfNamespace)
      local namespaceName
    ...
    FillExternalTypeName = function (sb, typeSymbol, funcOfNamespace, funcOfTypeName, transfor)
      ...
      if externalType ~= nil then
        if transfor ~= nil then
          ...
          local default
          default, classIdentifier = curTypeDeclaration:CheckTypeName(externalType)
          if curTypeDeclaration ~= nil and default then
        ...
        local typeName = extern or externalType:getName()
        sb:Append(typeName)
    ...
    GetTypeShortName = function (typeSymbol, funcOfNamespace, funcOfTypeName, transfor)
      local sb = SystemText.StringBuilder()
      ...
      local typeName = default or typeSymbol:getName()
      sb:Append(typeName)
    ...
    IsCollectionType = function (symbol)
      local containingNamespace = symbol:getContainingNamespace()
      while not containingNamespace:getIsGlobalNamespace() do
        if containingNamespace:getName() == "Collections" and containingNamespace:getContainingNamespace():getName() == "System" then
          return true
        end
